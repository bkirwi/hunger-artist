...
import org.apache.kafka.common.requests.{FetchRequest, MetadataRequest, ProduceRequest}
...
object TestScript {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(result)
    val produced = client.produce(new ProduceRequest(-1, 1000, Map(new TopicPartition("foo", 0) -> buffer).asJava)).get()
    println(produced)
    val fetched = client.fetch(new FetchRequest(100, 0, Map(new TopicPartition("foo", 0) -> new PartitionData(0, 1000000)).asJava)).get()
    println(fetched)
  }
}
